...
async def create_new(type, ctx, **kwargs):
    ...
    if has_pages or len(emoji) > 0:
        ...
        try:
            ...
            await ctx.send(f"{Emoji.get_chat_emoji('WARNING')} I am missing the permissions to add {Emoji.get_chat_emoji('LEFT')} and {Emoji.get_chat_emoji('RIGHT')} emoji for page navigation, please grant me reaction and external emoji permissions and try again")
...
def save_to_disc():
    Utils.save_to_disk("known_messages", known_messages)
